...
exports.create = async (req, res) => {
  try {
    ...
    try {
      ...
      // Si ya existe el usuario, ignorar duplicado (Postgres 23505)
      if (e && e.code !== "23505") {
...
exports.markAttendance = async (req, res) => {
  try {
    ...
    if (!courseCheckResult.rows.length) {
      return res
        .status(403)
        .json({
    ...
    const enrollmentCheckResult = await db.query(
      `
      SELECT e.id
      FROM enrollments e
    ...
    if (!enrollmentCheckResult.rows.length) {
      return res
        .status(400)
        .json({
          message:
            "El estudiante no tiene una matrícula aceptada en este curso",
        });
    }
    ...
    if (status === "ausente") {
      ...
      if (parseInt(absencesResult.rows[0].count) >= 3) {
        const studentResult = await db.query(
          "SELECT * FROM students WHERE id = $1",
          [student_id]
        );
        if (studentResult.rows.length && studentResult.rows[0].parent_phone) {
          try {
            await sendNotificationToParent(
              student_id,
              studentResult.rows[0].parent_phone,
              `Su hijo/a ${studentResult.rows[0].first_name} ${studentResult.rows[0].last_name} ha acumulado ${absencesResult.rows[0].count} faltas en este horario`,
              "absences_3"
            );
          } catch (notifErr) {
